...
export default function Navbar() {
    ...
    // Effect to close notification and main desktop dropdowns when clicking outside
    useEffect(() => {
        const closeOnOutsideClick = (e: MouseEvent) => {
            // Check if the click is outside the notification dropdown
            if (notifOpen && !(e.target as HTMLElement).closest('.notification-dropdown-container')) {
                setNotifOpen(false);
            }
            // Close desktop dropdowns
            if (openDropdown && !(e.target as HTMLElement).closest('.desktop-dropdown-container')) {
        ...
        <>
            ...
            >
                ...
                <div
                    className={`bg-gray-800 text-white w-full border-b border-gray-700`}
                >
                    {/* MODIFIED: Increased horizontal padding to px-3 for small mobile screens */}
                    <div className="w-full px-3 sm:px-4 py-0"> 
                        
                        {/* TEXT SIZE MODIFIED: text-[0.65rem] for 320px readability */}
                        <div className="flex justify-between items-center text-[0.65rem] sm:text-xs">
                            
                            {/* ⬅️ LEFT SIDE CONTENT - Email + Phone/Icons */}
                            <div className="flex items-center gap-2 sm:gap-3 justify-between sm:justify-start">
                                
                                {/* 📧 Email Text: Mobile पर hidden, Tablet (sm:) से दिखना शुरू */}
                                <div className="hidden sm:flex items-center gap-3"> 
                                    <div className="flex items-center gap-1.5"> {/* Gap reduced */}
                                        <Mail className="w-3 h-3 flex-shrink-0" /> {/* Icon size reduced */}
                                        <a
                                            href="mailto:info@atsglobaltech.in"
                                            className="hover:text-orange-300 transition" 
                                        >
                                ...
                                <div className="flex items-center space-x-2 sm:space-x-3 flex-shrink-0"> 
                                    ...
                                    <a
                                        ...
                                        className="hover:text-orange-300 transition flex items-center sm:hidden" 
                                    >
                                        <Mail className="w-3.5 h-3.5 flex-shrink-0" /> {/* Icon size reduced for mobile */}
                                    </a>
                                    
                                    {/* WhatsApp Icon (Hidden on sm and below for space on 320px) */}
                                    <a
                                        href="https://wa.me/919929825003"
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="hover:text-green-300 transition hidden lg:block" // Fully hidden until large screens
                                    >
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="12" // Size reduced for 320px
                                            height="12" // Size reduced for 320px
                                            viewBox="0 0 24 24"
                                            fill="currentColor"
                                        >
                                            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                                    ...
                                    {/* 📞 Phone Number Icon/Text (Text visible from Tablet (sm:)) */}
                                    <a
                                        href="tel:+919929825003"
                                        className="hover:text-orange-300 transition flex items-center gap-1 whitespace-nowrap"
                                    >
                                ...
                                <div className="relative pl-3 border-l border-gray-700 flex-shrink-0 notification-dropdown-container">
                                    <button
                                        onClick={(e: React.MouseEvent) => {
                                            ...
                                            setNotifOpen((prev) => {
                                                const newState = !prev;
                                                // Only reset count if opening the notification panel
                                                if (newState) setCount(0); 
                                                return newState;
                                            });
                                        }}
                ...
                >
                    ...
                    <div className="w-full px-3 sm:px-4">
                        {/* MAIN ROW */}
                        {/* 🎯 CHANGED: flex justify-between का उपयोग किया गया है ताकि लोगो लेफ्ट में और बटन राइट में हों (समान गैप)। */}
                        <div className="flex justify-between items-center h-16 sm:h-20"> 
                            ...
                            <div className="flex items-center flex-shrink-0"> 
                                ...
                                <Link to="/" className="flex items-center gap-2 sm:gap-6">
                                    ...
                                    {/* 🎯 RESPONSIVE LOGO TEXT SECTION (ml-2 added for 8px shift) */}
                                    <div className="flex flex-col ml-2"> {/* 🎯 ADDED ml-2 */}
                                        {/* ATS GLOBAL TECH */}
                                        {/* text-center यथावत रहेगा */}
                            ...
                            {/* RIGHT → MENU ITEMS (Desktop) - Hidden on mobile/tablet */}
...
function NavLink({ label, to, active = false, onClick }: { label: string, to: string, active?: boolean, onClick: () => void }) {
    return (
        <Link
            to={to}
            onClick={onClick}
            // Text size reduced for laptop view
